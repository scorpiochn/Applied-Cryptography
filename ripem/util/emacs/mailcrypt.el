;; mailcrypt.el v1.3, mail encryption with RIPEM and PGP
;; This is mailcrypt v1.3, an easy to use interface to RIPEM and/or PGP.
;; It is emacs19-centric, but can probably be made to work with other
;; emacsen with little trouble. 
...
;; 5-Dec-1993|Version 1.3|~/interfaces/mailcrypt.el.Z|
...
;; Currently supported modes are RMAIL, VM, and GNUS. Check out the section
;; on mode specific functions to see what you can bind.
...
(defun mc-create-read-menu-bar ()
  ...
  (let ((decrypt (nth 0 (cdr (assoc major-mode mc-modes-alist))))
    ...
    (local-set-key [menu-bar mailcrypt snarf]
		   (cons "Snarf Public Key" 'mc-snarf-keys))))
